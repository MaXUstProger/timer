...
function getTimeRemaining(deadline) {
  ...
  //в миллисекундах.
    var t = Date.parse(deadline) - Date.parse(new Date()); //вычитаем одну дату из другой
...
function initializeClock(id, deadline) {
  ...
  function updateClock() {
    ...
    if (t.total <= 0) {
      ...
      //скрываем блок с таймером и заголовком
      document.getElementById("countdown").classList.add('hidden');
      document.getElementById("countdown-title").classList.add('hidden');
      //меняем значение класса и появляется другой заголовок
      document.getElementById("countdown-title-2").className = 'active';
